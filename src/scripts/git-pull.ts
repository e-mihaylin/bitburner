...
export const main = async (ns: NS): Promise<void> => {
  ...
  const homeFiles: string[] = ns.ls('home')
    .filter((file: string) => extensions.includes(file.split('.').pop()!));
  const targetFiles: string[] = [...newFiles, ...homeFiles]
    .filter((file: string) => !excludeFiles.includes(file));
  for (const filePath of targetFiles) {
    ...
    if (await ns.wget(`${remoteFilePath}?ts=${new Date().getTime()}`, filePath)) {
      ...
      ns.tprint(`ERROR: ${filePath} was not updated. (Currently running or not located at ${remoteFilePath} )`);
    }
  }
}